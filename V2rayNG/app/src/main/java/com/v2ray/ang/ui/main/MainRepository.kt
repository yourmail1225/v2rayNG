...
AppConfig.MSG_STATE_STOP_SUCCESS -> MainServiceEvent.StateStopSuccess
                AppConfig.MSG_STATE_LOCK_DENIED -> MainServiceEvent.StateLockDenied(
    ...
    override fun encodeGroupLock(groupId: String, config: GroupLockConfig) {
        MmkvManager.encodeGroupLock(groupId, config)
    }

    override fun resetGroupUsedBytes(groupId: String) {
        MmkvManager.resetGroupUsedBytes(groupId)
    }

    override fun exportLockedPackage(guids: List<String>): String =
        AngConfigManager.exportLockedPackage(guids)

    override fun setClipboard(text: String) {
        Utils.setClipboard(localizedContext, text)
    }
